...
function Header() {
  ...
  const navLinks = [
    { to: "/apparel", label: t("client.nav.apparel") },
    { to: "/jewelry", label: t("client.nav.jewelry") },
    { to: "/accessories", label: t("client.nav.accessories") }
  ];
  ...
  return (
    <>
      {searchOpen && (
        ...
        >
          ...
          <div className="w-full max-w-2xl">
            <p className="text-[10px] tracking-[0.3em] uppercase text-[#5a5248] mb-4">{t("admin.search").toUpperCase()}</p>
            <input
              autoFocus
              value={searchQuery}
              ...
              placeholder={t("client.nav.search_placeholder")}
            ...
            {searchQuery.length > 1 && searchResults.length === 0 && (
              <p className="mt-6 text-sm text-[#5a5248]">
                {t("admin.no_data")} "{searchQuery}"
              </p>
            )}
          </div>
        </div>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 bg-[#0e0e0e] z-[150] flex flex-col animate-fade-in md:hidden">
          <div className="flex justify-between items-center px-6 py-5 border-b border-[#2a2820]">
            <Link to="/" className="text-xl font-serif tracking-[0.3em] text-[#c9a96e]">FASHY</Link>
          ...
          <nav className="flex flex-col p-8 gap-6">
            ...
            <div className="mt-8 pt-8 border-t border-[#2a2820] flex flex-col gap-4">
              <Link to="/login" onClick={() => setMobileOpen(false)} className="text-sm text-[#8a8070] tracking-widest uppercase">{t("client.nav.login")}</Link>
              <Link to="/register" onClick={() => setMobileOpen(false)} className="text-sm text-[#8a8070] tracking-widest uppercase">{t("client.nav.register")}</Link>
            </div>
          </nav>
        </div>
      )}
        ...
        <div className="flex items-center justify-between px-6 md:px-12 h-[68px]">
          ...
          <div className="flex items-center gap-4">
            ...
            >
              <ShoppingBag size={18} />
              {totalItems > 0 && (
                <span className="absolute -top-1.5 -right-1.5 w-4 h-4 bg-[#c9a96e] text-[#1a1208] text-[9px] font-bold rounded-full flex items-center justify-center">
            ...
            <button 
              ...
              className="text-[10px] tracking-widest font-bold text-[#c9a96e] hover:text-[#e8e2d9] transition-colors hidden md:block border border-[#c9a96e]/30 px-2 py-1"
            >
